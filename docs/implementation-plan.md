...
- Added command-level black-box tests in `cmd/doctor_test.go` for:
  ...
  - plain `doctor` refusing to prompt or write `.wsx.env` without `--fix`
  - interactive `--fix` resolution that writes `.wsx.env`
  - `doctor --fix --json` keeping stdout machine-readable
  - repointed workspace links being reported as broken
- Implemented the initial doctor behavior for portability and teammate onboarding:
  ...
  - detects unresolved `${VAR}` placeholders and only prompts when `--fix` is set and stdin is interactive
  - writes resolved variables back to `.wsx.env` only through `--fix`
  - sends `--fix --json` prompt/save messages to stderr so stdout stays valid JSON
  - checks duplicate names, case-collision risks, workspace nesting, nested refs, live links, live link target correctness, and non-git warnings
...
- Unresolved variables are prompted for only when `--fix` is set and stdin is interactive; all other runs report them as errors and exit non-zero.
- `wsx doctor --fix` requires an interactive terminal and errors immediately otherwise.
- `wsx doctor --fix --json` keeps stdout strictly JSON and sends prompt/save chatter to stderr.
- `wsx list` and `wsx doctor` only report a workspace link as healthy when the on-disk link resolves to the configured repo path.
...
- Added `.github/workflows/release.yml` to publish on Git tag pushes matching `v*`.
- Added `.goreleaser.yaml` so tagged releases build archives for:
  - Windows `amd64`, `arm64`
  - Linux `amd64`, `arm64`
  - macOS `amd64`, `arm64`
- Configured release builds to:
  - run `go test ./...` before publishing
  - emit GitHub release archives plus `checksums.txt`
- Updated `README.md` to document:
  - `go install github.com/jwolf/wsx@latest` as the default cross-platform install path
  - the tag-based GitHub release flow
  - the staged packaging rollout after GitHub Releases

**Frozen contracts after this slice:**

- Publishing a tag like `v0.1.0` triggers a GitHub Actions release workflow.
- Release artifacts are generated by GoReleaser from the repo root and uploaded to GitHub Releases.
- The first supported global install path is `go install github.com/jwolf/wsx@latest`; package-manager support remains a later distribution phase.
